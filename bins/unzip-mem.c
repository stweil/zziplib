...
static void
zzip_mem_entry_test(ZZIP_MEM_DISK* disk, ZZIP_MEM_ENTRY* entry)
{
    ZZIP_DISK_FILE* file = zzip_mem_entry_fopen(disk, entry);
...
static void
zzip_mem_entry_test_start(void)
{
    test_errors = 0;
    printf("Archive: %s\n", archive);
}
...
static char*
_zzip_ctime(const time_t* timeval)
{
...
static void
zzip_mem_entry_direntry_start(void)
{
    sum_usize = 0;
    sum_csize = 0;
    sum_files = 0;
...
static void
zzip_mem_entry_direntry_done(void)
{
    char exp = ' ';
...
static void
zzip_mem_entry_direntry(ZZIP_MEM_ENTRY* entry)
{
...
static void
zzip_mem_entry_listfiles(ZZIP_MEM_DISK* disk, char* filespec)
{
...
static void
zzip_mem_entry_listall(ZZIP_MEM_DISK* disk)
{
    zzip_mem_entry_direntry_start();
...
static void
zzip_mem_entry_testfiles(ZZIP_MEM_DISK* disk, char* filespec)
{
...
static void
zzip_mem_entry_testall(ZZIP_MEM_DISK* disk)
{
    zzip_mem_entry_test_start();
    ___ ZZIP_MEM_ENTRY* entry = zzip_mem_disk_findfirst(disk);
    for (; entry; entry = zzip_mem_disk_findnext(disk, entry))
...
static void
zzip_mem_entry_pipefiles(ZZIP_MEM_DISK* disk, char* filespec)
{
...
static void
zzip_mem_entry_pipeall(ZZIP_MEM_DISK* disk)
{
...
static void
zzip_mem_entry_makefiles(ZZIP_MEM_DISK* disk, char* filespec)
{
    ZZIP_MEM_ENTRY* entry = 0;
    while ((entry = zzip_mem_disk_findmatch(disk, filespec, entry, 0, 0)))
...
static void
zzip_mem_entry_makeall(ZZIP_MEM_DISK* disk)
{
    ZZIP_MEM_ENTRY* entry = zzip_mem_disk_findfirst(disk);
    for (; entry; entry = zzip_mem_disk_findnext(disk, entry))
        zzip_mem_entry_make(disk, entry);
